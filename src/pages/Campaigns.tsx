...
export default function Campaigns({ session }: CampaignsProps) {
  ...
  const handleSendCampaign = async (campaign: Campaign) => {
    ...
    const targetedClients = clients.filter(
      (client) =>
        client.email &&
        client.group_name &&
        client.group_name.toLowerCase() === campaign.target_group?.toLowerCase()
    );

    if (targetedClients.length === 0) {
      alert("Aucun client avec email trouvé dans ce groupe.");
      return;
    }

    const confirmSend = confirm(
      `Envoyer cette campagne à ${targetedClients.length} client(s) ?`
    );
